...
import {
  SetPasswordAction,
  SetPasswordMessage,
} from '../../../../../src/core/blocs/set-password/SetPasswordMessage';
import { SetPasswordBloc } from '../../../../../src/core/blocs/set-password/SetPasswordBloc';
import { SetPasswordRepoImpl } from '../../../../../src/shell/repositories/set-password/SetPasswordRepoImpl';
import { User } from '../../../../../src/core/models/User';
import { appCustomizations } from './SetPasswordTestData';
...
describe('SetPasswordBloc Unit', function () {
  let bloc: SetPasswordBloc;
  const routerService = new RouterServiceMockImpl();
  const testActivationToken = '1234cbvcbvx12341bvc123b4v1c2b3v41234';
  beforeEach(() => {
  ...
  describe('on INITIALIZE', function () {
    it('it initializes correctly', function (done) {
      const defaultMessage = new SetPasswordMessage();
      const initializationMessage = new SetPasswordMessage();
      initializationMessage.status = 'PROCESSING';
      initializationMessage.action = new SetPasswordAction({
        type: 'INITIALIZE',
        payload: {},
      });
      ...
      const messages: SetPasswordMessage[] = [];
      (bloc.outbox() as Observable<SetPasswordMessage>)
        .pipe(take(3))
        .subscribe({
          next: (message: SetPasswordMessage) => {
  ...
  describe('on VALIDATE', function () {
    describe('validators()', function () {
      it('it returns FORM_REQUIRED_INPUT when password is missing', function () {
        const message = new SetPasswordMessage();
    ...
    it('it changes button status to ENABLED if form is valid', function (done) {
      const defaultMessage = new SetPasswordMessage();
      const validationValidMessage = new SetPasswordMessage();
      validationValidMessage.state.form = {
        password: 'survv123',
        confirmationPassword: 'survv123',
      };
      validationValidMessage.action = new SetPasswordAction({
        type: 'VALIDATE',
        payload: {
      ...
      const validationInvalidMessage = new SetPasswordMessage();
      validationInvalidMessage.state.form = {
        password: 'test',
        confirmationPassword: '',
      };
      validationInvalidMessage.action = new SetPasswordAction({
        type: 'VALIDATE',
        payload: {
  ...
  describe('on SWITCH_LANGUAGE', function () {
    it('it switches language correctly', function (done) {
      const defaultMessage = new SetPasswordMessage();
      const switchLanguageMessage = new SetPasswordMessage();
      switchLanguageMessage.action = new SetPasswordAction({
  ...
  describe('on SET_PASSWORD', function () {
    it('it sets password and logs in successfully', function (done) {
      const defaultMessage = new SetPasswordMessage();

      const validationMessage = new SetPasswordMessage();
      validationMessage.state.form = {
        password: 'survv123',
        confirmationPassword: 'survv123',
      };
      validationMessage.action = new SetPasswordAction({
        type: 'VALIDATE',
        payload: {
          form: { password: 'survv123', confirmationPassword: 'survv123' },
        },
      ...
      setPasswordMessage.action = new SetPasswordAction({
        type: 'SET_PASSWORD',
        payload: {},
      });
      setPasswordMessage.buttonStatus = 'PROCESSING';

      const setPasswordDoneMessage = setPasswordMessage.clone();

      $sb.stub(SetPasswordRepoImpl.prototype, 'setPassword').resolves(
        new User({
          id: 1234,
          vendorId: 1234,
          email: 'test@test.com',
    ...
    it('it handles set password failure correctly', function (done) {
      ...
      $sb
        .stub(SetPasswordRepoImpl.prototype, 'setPassword')
        .rejects(new Error('error'));
...
describe('SetPasswordBloc Integration', function () {
  let bloc: SetPasswordBloc;
  const routerService = new RouterServiceMockImpl();

  beforeEach(() => {
    bloc = new SetPasswordBloc({
      activationToken: '12341234asdfasf123423',
      setPasswordRepo: new SetPasswordRepoImpl(),
      userPreferenceRepo,
      notificationService,
      routerService,
  ...
  it('it sets password successfully', function (done) {
    ...
    const setPasswordRequestPromise = wiremock
      .stub<CallCenterUserActivationRequest, CallCenterUserActivationResponse>()
      .request({
