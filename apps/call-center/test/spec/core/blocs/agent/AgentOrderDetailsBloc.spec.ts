...
} from '../../../../../src/core/blocs/agent/AgentOrderDetailsMessage';
import { AgentOrderDetailsBloc } from '../../../../../src/core/blocs/agent/AgentOrderDetailsBloc';
import {
  AssignOrderRequest,
  AssignOrderResponse,
...
} from './AgentOrderDetailsBlocTestData';
...
describe('AgentOrderDetailsBloc Unit', function () {
  let bloc: AgentOrderDetailsBloc;
  ...
  describe('on INITIALIZE', function () {
    it('initializes correctly when order is assigned to current user', function (done) {
      const {
        defaultMessage,
      ...
      const messages: AgentOrderDetailsMessage[] = [];
      (bloc.outbox() as Observable<AgentOrderDetailsMessage>)
        .pipe(take(4))
        .subscribe({
          next: (message: AgentOrderDetailsMessage) => {
            ...
            expect(
              messages.some((message) => {
                return (
                  ...
                  message.assignedStatus === 'ASSIGNED_TO_ME' &&
                  deepEqual(
                    message.state.order,
    ...
    it('initializes correctly when order is not assigned to current user', function (done) {
      const {
        defaultMessage,
        initializeLoadingMessage,
        initializeOrderDetailsDoneMessage,
      ...
      $sb
        .stub(OrdersRepoImpl.prototype, 'getOrderJourney')
        .resolves(mappedOrderJourneyStub());
      $sb.stub(kvStorage, 'getItem').withArgs('user-id').resolves(0);
          ...
          complete: () => {
            ...
            expect(
              messages.some((message) => {
                return (
                  message.orderStatus === 'IDLE' &&
                  message.assignedStatus === 'NOT_ASSIGNED_TO_ME' &&
                  deepEqual(
                    message.state.order,
                    initializeOrderDetailsDoneMessage.state.order
                  )
  ...
  describe('on ASSIGN_ORDER', function () {
    it('assigns the order successfully', function (done) {
      $sb
        .stub(OrdersRepoImpl.prototype, 'getOrder')
        .resolves(mappedOrderDetailsStub());
      $sb
        .stub(OrdersRepoImpl.prototype, 'getOrderJourney')
        .resolves(mappedOrderJourneyStub());
      $sb.stub(OrdersRepoImpl.prototype, 'assignOrder').resolves();
        ...
        .subscribe({
          ...
          complete: () => {
            ...
            expect(
              messages[1].orderStatus === 'LOADING' &&
                messages[1].journeyStatus === 'LOADING' &&
                messages[1].action.type === 'ASSIGN_ORDER'
            ).to.be.true;
            expect(
              messages.some((message) => {
                return message.orderStatus === 'IDLE';
    ...
    it('handles assigning order failure successfully', function (done) {
      $sb.stub(OrdersRepoImpl.prototype, 'assignOrder').rejects();
...
describe('AgentOrderDetailsBloc Integration', function () {
  let bloc: AgentOrderDetailsBloc;
  const dummyUserId = 123;
  const dummyOrderId = 123;
  const dummyVendorId = 123;
  ...
  it('assigns order correctly', function (done) {
    const {
      initializeOrderDetailsDoneMessage,
      initializeOrderJourneyDoneMessage,
    } =
    ...
    $sb
      .stub(kvStorage, 'getItem')
      .withArgs('vendor-id')
      .resolves(dummyVendorId)
      .withArgs('user-id')
      .resolves(dummyUserId);
    ...
    const assignOrderMockPromise = wiremock
      .stub<AssignOrderRequest, AssignOrderResponse>()
      .request({
        requestLine: 'post /consumer/api/v1/vendors/:vendorId/assign-order',
        params: { vendorId: dummyVendorId },
        body: {
          orderId: dummyOrderId,
        },
      })
              ...
              expect(
                messages.some((message) => {
                  return (
                    message.orderStatus === 'LOADING' &&
                    message.journeyStatus === 'LOADING' &&
                    message.action.type === 'ASSIGN_ORDER'
                  );
                })
              ).to.be.true;
              expect(
  ...
  it('hydrates rejection reasons correctly', function (done) {
    const defaultMessage = new AgentOrderDetailsMessage();

    const openRejectionFormMessage = new AgentOrderDetailsMessage();
    openRejectionFormMessage.rejectFormStatus = 'OPENED';
    openRejectionFormMessage.action = new AgentOrderDetailsAction({
      type: 'OPEN_REJECT_FORM',
    });
