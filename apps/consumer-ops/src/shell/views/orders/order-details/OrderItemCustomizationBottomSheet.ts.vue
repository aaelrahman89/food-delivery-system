      ...
      >
        ...
        <template v-else>
          ...
          >
            ...
            <div
              v-for="(selection, selectionIndex) in option.selections"
              :key="selection.id"
            >
              <div
                v-for="(
                  relatedOption, relatedOptionIndex
                ) in selection.relatedOptions"
                :key="relatedOption.optionId"
              >
                <div
                  v-if="
                    pm.isActiveCustomization({
                  ...
                  "
                  class="options-panel__related"
                >
                  <div class="options-panel__related__header">
                    {{ $t(relatedOption.selectionOptionName) }} -
                    {{ $t(relatedOption.selectionName) }}
                  </div>
                  <vg-panel
                    class="item-customization-bottom-sheet__item-option"
                    dark
                    ...
                    :collapsed="
                      pm.isCollapsedRelatedOption({
                        ...
                        relatedOptionIndex: relatedOptionIndex,
                      })
                    "
                    @toggle="
                  ...
                  >
                    <template #header>
                      <vg-flex gap-size="mid" align-items="center">
                        <vg-svg
                          :src="SVG_ICON_CIRCLE_CHECK"
                          :fill="
                            getRelatedOptionCheckMarkColor({
                              optionIndex: index,
                              selectionIndex: selectionIndex,
                              relatedOptionIndex: relatedOptionIndex,
                            })
                        ...
                        <div
                          ...
                          "
                        >
                          {{ $t(relatedOption.displayName) }}
                        </div>
                        <div
                          v-if="relatedOption.mandatory"
                    ...
                    >
                      <div class="selection-item">
                        <template v-if="relatedOption.multiSelection">
                          {{
                            $t('ITEM_CUSTOMIZATION_SELECTION_HEADER_MULTI', {
                              min: relatedOption.minAllowed,
                              max: relatedOption.maxAllowed,
                            })
                          }}
                        </template>
                        <template v-else>
                      ...
                      <div
                        v-for="relatedOptionSelection in relatedOption.selections"
                        :key="relatedOptionSelection.id"
                        class="selection-item"
                      >
                        <vg-checkbox
                          v-if="relatedOption.multiSelection"
                          :label="$t(relatedOptionSelection.displayName)"
                          hide-details
                          :value="
                            pm.isActiveRelatedOptionSelection({
                              selectionId: selection.id,
                              relatedOptionSelectionId:
                                relatedOptionSelection.id,
                            })
                          "
                          class="selection-item__checkbox"
                          @input="
                            pm.onRelatedOptionMultiSelectionChange({
                              optionId: option.id,
                              selectionId: selection.id,
                        ...
                        <vg-radio
                          ...
                          @input="
                            pm.onRelatedOptionSingleSelectionChange({
                              optionId: relatedOption.selectionOptionId,
                              selectionId: relatedOption.selectionId,
                              relatedOptionId: relatedOption.optionId,
                              relatedOptionSelectionId:
                                relatedOptionSelection.id,
                              checked: $event,
                        ...
                        <div class="vg-text-primary">
                          {{ relatedOptionSelection.price.toString() }}
                          {{ $t(relatedOptionSelection.price.currency) }}
                        </div>
                      </div>
                    </div>
                  </vg-panel>
...
import Vue from 'vue';
import {
  CatalogueItemCustomizationPM,
  RelatedOptionIndex,
} from '../../../../core/presentation-models/orders/CatalogueItemCustomizationPM';
...
export default Vue.extend({
  name: 'OrderItemCustomizationBottomSheet',
  components: {
    VgOverlay,
    VgButton,
    VgCheckbox,
  ...
  methods: {
    ...
    getRelatedOptionCheckMarkColor(index: RelatedOptionIndex): string {
      if (this.pm.isCompletedRelatedOption(index)) {
        return this.completedOptionFill;
      }
      return this.incompleteOptionFill;
    },
