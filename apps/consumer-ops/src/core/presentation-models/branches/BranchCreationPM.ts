...
export class BranchCreationPM extends BasePM {
  ...
  constructor(options: BranchCreationPMOptions) {
    super();
    ...
    this.form
      .assignSubmitHandler(async () => {
        await this._longProcess(async () => {
          await this._branchRepo.createBranch(this.vendorId, this.form);
        });
      })
...
interface BranchCreationPMOptions {
  vendorId: number;
