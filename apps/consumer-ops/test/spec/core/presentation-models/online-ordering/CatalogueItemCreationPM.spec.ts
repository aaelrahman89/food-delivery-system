...
import { CatalogueItemCreationPM } from '../../../../../src/core/presentation-models/online-ordering/CatalogueItemCreationPM';
...
import {
  ...
  catalogueItemCreationResponseStub,
...
describe('CatalogueItemCreationPM', function () {
  it('hydrates the vendor and catalogue', async function () {
    const pm = new CatalogueItemCreationPM({
    ...
    await wiremock
      ...
      .request({
        ...
        params: { catalogueId: 123 },
      })
      .response({ status: 200, body: catalogueDetailsResponseStub() });
  ...
  it('submits successfully if form can be submitted & notifies successfulOperation & returns true', async function () {
    ...
    const itemCreationStub = catalogueItemCreationResponseStub();
    ...
    await wiremock
      ...
      .response({ status: 200, body: itemCreationStub });
    ...
    $sb.assert.calledWith(uploadGalleryStub, {
      entityId: itemCreationStub.itemId,
  ...
  it('notifies the error to notification service if failed & returns false', async function () {
    ...
    $sb
      .stub(CatalogueItemsRepoImpl.prototype, 'createItem')
      .rejects(new LocalError({ code: 'ERROR', message: 'an error' }));
