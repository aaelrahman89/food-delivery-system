...
import { BranchCreationPM } from '../../../../../src/core/presentation-models/branches/BranchCreationPM';
...
import { mapBranchToBranchCreationRequest } from '../../../../../src/shell/repositories/restaurants/branches/mappers/requests';
...
describe('BranchCreationPM', function () {
  ...
  it('Should handle cities hydration errors successfully', async function () {
    ...
    await pm.hydrateCities(
      mapCountryListingResponseToCountries(countriesListResponseStub())[0].id
    );
    assert.deepEqual(
      notificationService.notification,
      createNotification(errModel)
  ...
  it('Should handle areas hydration errors successfully', async function () {
    ...
    await pm.hydrateAreas(
      mapCityListingResponseToCities(citiesListV2ResponseStub())[0].id
    );
    assert.deepEqual(
      notificationService.notification,
      createNotification(errModel)
  ...
  it('Should hydrate cities successfully', async function () {
    ...
    await pm.hydrateCities(21234123937831514000);
    assert.deepEqual(
      pm.cities,
  ...
  it('Should hydrate areas successfully', async function () {
    ...
    await pm.hydrateAreas(21234123937831514000);
    assert.deepEqual(
      pm.areas,
  ...
  it('Should handle map actions successfully', async function () {
    ...
    pm.form.address.coordinates = [29, 28];
    pm.openMapModal();
    assert.isTrue(pm.shouldShowMapModal);
    assert.deepEqual(pm.mapConfig.point, pm.form.address.coordinates);
  ...
  it('Should handle form submitting errors successfully', async function () {
    ...
    $sb
      .stub(BranchRepoImpl.prototype, 'createBranch')
      .rejects(new Error('anything'));
