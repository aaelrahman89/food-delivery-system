...
import { UsersListAction, UsersListMessage } from './UsersListMessage';
...
export class UsersListBloc extends BaseBloc<UsersListAction, UsersListMessage> {
  private readonly _usersRepo: UsersRepo;
  private readonly _routerService: RouterService;
  private readonly _localizationService: LocalizationService;
  private readonly _notificationService: NotificationService;

  protected _message: UsersListMessage;

  protected readonly outbox$ = new BehaviorSubject<UsersListMessage>(
    new UsersListMessage()
  );

  protected readonly inbox$ = new BehaviorSubject<UsersListAction>(
    new UsersListAction({ type: 'NONE', payload: {} })
  );
  ...
  constructor({
    ...
    this._message = new UsersListMessage();

    this.inbox().subscribe((action: UsersListAction) => {
      this.handleActions(action);
    });
  }

  protected handleActions(action: UsersListAction): void {
    if (action.type === 'NONE') {
      return;
    }
    ...
    switch (action.type) {
      ...
      case 'DEACTIVATE_USER':
        ...
        this._message = newMessage;
        this.outbox$.next(this._message);
        this.deactivateUser(newMessage.state.selectedUser.id);
        break;
      case 'UPDATE_USER':
        newMessage.action = action;
  ...
  isUpdateValid(message: UsersListMessage): boolean {
    return Object.entries(message.updateValidators()).every(
      ([, fn]) => fn() === true
    );
  }

  isCreationValid(message: UsersListMessage): boolean {
